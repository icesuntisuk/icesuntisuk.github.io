...
1. WEP (Wired Equivalent Privacy) มีการเข้ารหัสโดยใช้ Algorithm RC4 ซึ่งเป็นการเข้ารหัสแบบ Symmetric Key Stream โดยจะมีขั้นตอนการใ้ช้งานระหว่าง Client และ Access Point ดังนี้
  ...
  - ช่องโหว่ของ WEP
    ...
    - ช่องโหว่ของ WEP นั้นเกิดจากที่ความยาวที่น้อยเกินไปของ IV และ password ที่คงที่ ซึ่ง WEP นั้นเมื่อเวลาผ่านไปนานๆจะมี data packet ที่ใช้ IV ซ้ำของเก่านั่นเอง เมื่อผู้โจมตีรวบรวมข้อมูลได้ระยะหนึ่งที่ได้ IV เดียวกันหลายๆ packet ก็จะทำให้สามารถหา password ได้ อีกทั้งยังมีโปรแกรมหลายตัวสำหรับการโจมตี WEP เช่น aircrack-ng , gerix , airsnort เป็นต้น โดยวิธีการโจมตีนั้นจะทำดังนี้
    ...
    ```
2. WPA นั้นถูกพัฒนาเพื่อแก้ไขจุดอ่อนของ WEP มีความปลอดภัยกว่า WEP เพราะใช้ Algorithm ที่แข็งแกร่งกว่า RC4 ซึ่ง WPA นั้นมี Algorithm หลายตัวให้เลือกเช่น TKIP (Temporal Key Integrity Protocol) โดย WPA นั้นมีการเข้ารหัสแบบ Dynamic Key Distribution และรวมถึงการตรวจสอบการเข้าใช้ (Authentication)โดยใช้ 4-Ways Handshake ซึ่งใช้มาตรฐาน IEEE802.1x โดยการทำงานของ WPA นั้นจะเป็นแบบ WPA Pre-Shared Key ซึ่งมีระดับความปลอดภัยหลายแบบ เช่น
     - WPA Personal (WPA-PSK) ใช้ Pre Share Key (PSK) เป็น key ที่ต้องแจกจ่ายให้กับผู้ใช้ทุกคน เพื่อใช้สำหรับการยืนยันตัวตน
     - WPA Enterprise (WPA+RADIUS) ซึ่งการเข้ารหัสนี้จะต้องมีการ Authentication ไปยัง Radius Server ก่อนด้วยการกรอก username และ password แล้วยังสามารถใช้ protocol EAP-TLS ในการติดต่อเพื่อเพิ่มการตรวจสอบด้วย Certificate ของผู้ใช้ที่ออกให้โดย CA
...
```bash
    ...
    airodump-ng -w wpa-file.cap -c 10 -b <MAC Access Point Victim> wlan0

  
    #  attemp replay packet to client 
    aireplay-ng -0 100 -a <MAC Access Point Victim> -c <MAC Client> wlan0
    
    # Bruteforce attack
    aircrack -w /usr/share/wordlists/rockyou.txt -b  wpa-file*.cap
```

3. WPA2 นั้นยังมีพื้นฐานมาจาก​ WPA (802.11i) โดยมีการเพิ่มความปลอดภัยเข้าไปเพิ่มเติมโดยเฉพาะการเข้ารหัสข้อมูล WPA2 นั้นจะทำงานโดยใช้ Cipher block Chaining Message Authentication Code Protocol (CCMP) และใช้ Advanced Encryption Standard (AES) ในการตรวจสอบความถูกต้องและเข้ารหัส โดย key อยู่ที่ 128bit
   - ช่องโหว่ KRACK ใน WPA2 คือ KRACK (Key Reinstallation Attack) เป็นช่องโหว่ที่ทำให้ผู้โจมตีสามารถทำ MITM ระหว่าง user กับ Access Point (AP) ได้รวมถึงบังคับให้ AP ทำการเข้ารหัสข้อมูลใหม่อีกครั้ง เมื่อเกิดการส่ง authentication package มาเรื่อยๆ ก็จะทำให้ผู้โจมตีสามารถนำไปถอดรหัสผ่านด้วย wordlist ได้ในที่สุด เงื่อนไขที่จะทำการโจมตี KRACK ได้คือ ผู้โจมตีต้องอยู่ในรัศมีของ Wifi ที่เป็นเป้าหมายและ client จากนั้นทำการสร้าง AP ปลอมที่บอกว่าตัวเองเป็น AP ตัวจริงขึ้นมา จากนั้นบังคับให้ client มาเชื่อมต่อกับตัวเอง ก่อนที่จะส่งข้อมูลให้กับ AP ตัวจริง โดยเมื่อ client จะส่ง message ที่ 4 ที่เป็นการจบการเชื่อมต่อ ตัว AP ปลอมก็จะปัดทิ้งทำให้ AP ต้องส่ง Group Temporal Key - GTK กลับมายัง client ซ้ำๆ เรื่อยๆ จนกระทั่งผู้โจมตีได้ข้อมูลที่มากพอนำไป crack ได้นั่นเอง 

4. WPA3 นั้นถูกเปิดตัวใช้งานใน 2018 ก็ยังมีการใช้งานที่แพร่หลายนัก WPA3 ถือเป็นวิธีการที่ปลอดภัยและเชื่อถือได้มากที่สุดในปี 2020 ทั้งนี้ได้มีการแก้ไขจุดบกพร่องของ WPA2 นั่นคือ four-way handshake ที่ไม่สมบูรณ์ทำให้การเชื่อมต่อ Wi-Fi มีความเสี่ยง มีคุณสมบัติเพิ่มเติมดังต่อไปนี้
   - แม้จะให้บริการเป็น Public Wifi ก็ยังปลอดภัย โดยปกติแล้วหากเราให้บริการ Public Wifi (Open network) ด้วยอุปกรณ์ที่รองรับ WEP,WPA,WPA2 นั้นจะมีการให้ข้อมูลแบบไม่เข้ารหัสแต่อย่างใด จึงทำให้อาจถูก sniff (ดักฟังข้อมูล) ได้ แต่ใน WPA3 การให้บริการในลักษณะ Public Wifi มีความปลอดภัยมากขึ้น เนื่องจากมีการเข้ารหัสแฝงไปกับข้อมูลด้วย
   - ป้องกันการ brute force โดยผู้ใช้งานทั่วไปหรือแม้แต่โปรแกรมถอดรหัส โดยปกติแล้วจากเนื้อหาที่ผ่านมาเราจะเห็นว่า WPA,WPA2 นั้นมีปัญหาในลักษณะ brute force password ได้ แต่ WPA3 นั้นมีการเพิ่มความสามารถในการป้องกัน brute force ได้ โดย feature นี้จะเป็นการตรวจจับ
   - การเชื่อมต่อกับอุปกรณ์ IoT ที่ง่ายขึ้น เนื่องจาก IoT ส่วนหนึ่งมักจะไม่มีหน้าจอในการกำหนดค่าใด ๆ จึงทำให้การ setting เป็นไปได้อย่างยากลำบาก ดังนั้นหลายๆค่า IoT จึงได้ทำโหมดการเชื่อมต่อในลักษณะของ WPS มาให้ ซึ่งแน่นอนว่าการเชื่อมต่อ WPS นั้นมีปัญหาทางด้านความปลอดภัยมาตลอด ไม่ว่าจะเป็นช่องโหว่ในลักษณะ firmware หรือรวมถึงการที่กำหนด WPS ไว้สั้นมากจนสามารถ brute force ได้เป็นต้น WPA3 จะรองรับ WPS ที่ปลอดภัยมากขึ้น เพื่อทำให้ IoT เชื่อมต่อได้ง่ายและปลอดภัยจึงทำให้ลดปัญหาความปลอดภัยของอุปกรณ์ IoT ได้อีกด้วย
  - การเชื่อมต่อที่ปรับปรุงเพิ่มจาก WPA2 โดย WPA3 นั้นปรับปรุงในเรื่อง Handshake เพื่อป้องกันการนำ key ไปถอดรหัสข้อความที่ถูกดักมาได้ด้วย  Handshake แบบ SAE (Simultaneous Authentication of Equals) ที่ป้องกันการเอาคีย์ที่ได้ทีหลังมาใช้ถอดรหัสข้อมูลเก่าที่เคยถูกดักฟังไว้ (Forward Secrecy)
  - การกำหนด Key ที่มากขึ้น โดยการเข้ารหัสบน WPA2/AES นั้นจะใช้งาน key ตั้งต้นที่ 128-bit แต่ WPA3 นั้นจะเปลี่ยนไปใช้ key ตั้งต้นที่ขนาด 192-bit ทำให้เกิดความปลอดภัย ยากต่อการคาดเดามากขึ้น
  - WPA3-Enterprise ยังคงให้บริการแบบเดียวกับ WPA2 แต่มีการปรับปรุงเพิ่มเติมในหลายๆส่วน ดังนี้
    - มีการ authentication ที่ใช้เป็น 256-bit Galois/Counter Mode Protocol (GCMP-256)
    - การสร้าง key ใช้เป็น 384-bit Hashed Message Authentication Mode (HMAC) โดยใช้ SHA (Secure Hash Algorithm)
    - ใช้งาน ECDH (Elliptic Curve Diffie-Hellman) และ ECDSA (Elliptic Curve Digital Signature Algorithm) ในกระบวนการตกลงแลกเปลี่ยน key
    - มีการทำ frame protection โดยใช้ 256-bit Broadcast/Multicast Integrity Protocol Galois Message Authentication Code (BIP-GMAC-256)
  - ช่องโหว่ Dragonblood นั้นเป็นช่องโหว่ที่ถูกพบใน WPA3 ซึ่งช่องโหว่นี้ประกอบไปด้วย 5 ช่องโหว่ย่อยนั่นคือ DoS, Downgrade Attack และ Side-channel Attack ซึ่งทำให้ผู้โจมตีสามารถที่จะ Recover password สำหรับการเชื่อมต่อและทำให้ผู้โจมตีสามารถที่จะเจาะเข้าไปในเครือข่ายของเป้าหมายได้
    - DOS ผู้โจมตีสามารถสร้าง initiate handshake จำนวนมากไปยัง Access Point (AP) ที่รองรับ WPA3 เพื่อให้เกิดการใช้งานทรัพยากรสูงจนเกิดการทำงานผิดพลาด
    - Downgrade Dictionary Attack ทาง Wi-Fi Alliance หรือผู้กำหนดมาตรฐานได้เพิ่ม Transition mode เข้าไปใน WPA3 เพื่อให้สามารถรองรับการทำงานของ WPA2 ได้ ซึ่งหากผู้โจมตีสร้าง Access Point ที่บอกว่ารองรับเฉพาะ WPA2 ขึ้นมา จะทำให้ client จำเป็นต้องกลับไปใช้ WPA2 4-way handshake ตาม WPA2 เดิม แต่อย่างที่ทราบกันดีว่า WPA2 นั้นมีช่องโหว่จึงทำให้ผู้โจมตีสามารถที่จะได้ key จาก client ในขณะที่ client เชื่อมต่อเป็นแบบ WPA2 นั่นเอง
    - Group Downgrade Attack ช่องโหว่นี้จะเกิดขึ้นเมื่อ WPA3 นั้นรองรับการทำงานที่ใช้งาน Elliptic Curves Algorithm แบบ P-521 และ P-256 ทางผู้โจมตีสามารถบังคับให้ AP ใช้งานเป็น P-256 ซึ่งมีความปลอดภัยน้อยกว่าได้ โดยการส่งสัญญาณกวนแล้วบอกซ้ำๆว่าตัวเองนั้นไม่ support P-521
    - Cache-based Side-channel attack (CVE-2019-9494) เป็นการใช้ช่องโหว่บน ‘hunting and pecking’ Algorithm ในโปรโตคอล Dragonfly ของ WPA3 โดยในกรณีที่หากผู้โจมตีสามารถรัน script code อันตรายในเครื่องของ client ที่เชื่อมต่อ WPA3 อยู่ได้ ผู้โจมตีสามารถตรวจสอบได้ว่า password generation algorithm ที่ถูกใช้ในฝั่งของ client คืออะไร จากนั้นค่อยทำการแกะรหัสผ่านอีกที
    - Timing-based Side-channel attack (CVE-2019-9494) เป็นการโจมตีใน ‘multiplicative groups’ feature ซึ่งกระทำในช่วง handshake ของโปรโตคอล Dragonfly เมื่อมีการใช้งานหลายๆกลุ่มแล้วมีการ encoding password โดยใช้ password encoding algorithm ผู้โจมตีสามารถเฝ้าสังเกตและตรวจสอบตัวเลขที่เกิดขึ้นซ้ำจาก password encoding algorithm จนนำไปสู่การเดา password ที่ถูก encode ได้สำเร็จ

5. Bluetooth เป็น network ที่ยังคงมีการใช้อยู่อย่างต่อเนื่อง แต่เริ่มน้อยลงในอุปกรณ์ต่างๆ Bluetooth เป็น wireless protocol ที่ใช้ในความกว้างที่ค่อนข้างจำกัดระหว่างอุปกรณ์ โดย Bluetooth จะทำงานใน ISM Band ที่ 2.4 Ghz. โดยเครื่องที่สื่อสารด้วย Bluetooth ที่เป็น Class A จะมีระยะประมาณ 100 เมตรโดยขึ้นอยู่กับความแรงของสัญญาณ แต่โดยปกติ Bluetooth จะทำงานที่ประมาณ 10 เมตรเท่านั้น ซึ่งทำให้ต้องใช้งานในระยะที่ใกล้มากๆนั่นเอง
   - Bluetooth นั้นมีความสามารถที่หลากหลายขึ้นอยู่กับลักษณะการทำงาน (profile) เช่น 
     - Advanced Audio Distribution Profile (A2DP) สำหรับการส่งสัญญาณเสียงแบบ stereo ระหว่างอุปกรณ์
     - Audio/Video Remote Control Profile (AVRCP) สำหรับการควบคุมภาพและเสียงผ่านอุปกรณ์ bluetooth
     - Dial-up Networking Profile (DUN) สำหรับการเชื่อมต่ออินเทอร์เน็ตผ่านอุปกรณ์ bluetooth
     - Hands-Free Profile (HFP) สำหรับการพูดคุยตลอดจนการควบคุมโทรศัพท์เบื้องต้น เช่น การรับสายโดยใช้อุปกรณ์ bluetooth เป็นต้น
     - Human Interface Device Profile (HID) สำหรับการเชื่อมต่อที่ปลอดภัยสำหรับอุปกรณ์รับ input เช่นคีย์บอร์ด เมาส์ เป็นต้น
  - Bluetooth Paring เมื่อเครื่อง 2 เครื่องนั้นเชื่อมต่อเข้าด้วยกัน (paired) จะเป็นการแชร์ข้อมูลระหว่างกันและกัน โดยจะบอกว่าลักษณะการทำงานอะไรที่เครื่องนั้นๆ support โดยขณะกำลังที่จะเชื่อมต่อเข้าด้วยกัน (Pairing Process) นั้นจำเป็นต้องมีการตรวจสอบซึ่งกันและกัน ทั้งคู่จะประกาศว่าให้ทำการเชื่อมต่อ ส่วนอีกฝ่ายจะเป็นการบอกว่าจะเชื่อมต่อ ในบางลักษณะการเชื่อมต่อจะกำหนดให้ทำการตรวจสอบ (Verify) ก่อนทำการเชื่อมต่อ เพื่อป้องกันการเชื่อมต่อที่ไม่พึงประสงค์ โดยจะให้แต่ละอุปกรณ์กรอกรหัสผ่าน (PIN) ในแต่ละอุปกรณ์ ซึ่งรหัสผ่านดังกล่าวมักจะแสดงในหน้าจอของอุปกรณ์นั้นๆ หรือบางรุ่นอาจจะกำหนดเป็นแบบตายตัวไว้ในอุปกรณ์แต่ละอุปกรณ์ (Hard-coded) ไปเลยนั่นเอง เมื่อรหัสผ่านของทั้งคู่กรอกมาตรงกัน จึงจะสามารถเชื่อมต่อกันได้สำเร็จ ใน Bluetooth v2.1 นั้นมีการเพิ่ม feature security เข้าไปนั่นคือ Simple Secure Pairing (SSP) โดย SSP จะเป็น feature ที่ถูกรับรองโดย Federal Information Processing Standard (FIPS) ทำให้มีกระบวนการเชื่อมต่อเพิ่มเติมอีก 3 ลักษณะ รวมกับวิธีการเก่าจะกลายเป็นการเชื่อมต่อ 4 รูปแบบการเชื่อมต่อ (mechanism)
    - การจับคู่แบบเดิม (legacy pairing) คืออุปกรณ์ทั้งคู่จะต้องป้อนรหัส (PIN code) ก่อนที่จะทำการเชื่อมต่อด้วยกัน หากอุปกรณ์ทั้งป้อนรหัสเหมือนกันถึงจะสามารถเชื่อมต่อได้ ใช้ตั้งแต่ v0 - v2.0
    -  Just Works เป็นการ pairing แบบที่ไม่ต้อง interact ใดๆ มักจะใช้ในอุปกรณ์ทางด้านการกีฬา
    -  Passkey เป็นการเชื่อมต่อที่มีจอแสดงผลและมี input ให้สามารถป้อนข้อมูลได้ โดยอุปกรณ์ฝั่งแรกจะแสดงตัวเลข 6 หลักขึ้นมาที่หน้าจอ จากนั้นนำตัวเลขดังกล่าวไป input กับอีกอุปกรณ์หนึ่ง
    -  LE Secure Connection จะเป็นการใช้งานเชื่อมต่อโดยใช้ Elliptic-curve Diffie–Hellman (ECDH) Protocol เพื่อแลกเปลี่ยนคีย์ระหว่างกัน ทำให้การแลกเปลี่ยน key นั้นปลอดภัยมากขึ้น โดยการเชื่อมต่อในลักษณะนี้จะใช้กับ Bluetooth v4.2 ขึ้นไป ผู้ใช้งานจะสามารถ pairing อุปกรณ์ร่วมกันได้ทั้งแบบ Just Work และ Passkey และมีวิธีที่เพิ่มขึ้นมาคือ Numeric จะเป็นการให้อุปกรณ์ทั้งสองแสดงตัวเลข 6 หลักบนหน้าจอ แล้วให้กรอกทั้งคู่เข้ากับทั้ง 2 อุปกรณ์เพื่อให้ยืนยีน
 - การโจมตีใน Bluetooth
   - Sniffing โดยใช้ Btscanner ซึ่งเป็นเครื่องมือที่ถูกออกแบบมาใช้สำหรับการโจมตี bluetooth โดยเฉพาะใน 2 ส่วนหลักๆคือ sniffing และ brute force นั่นเอง โดยเริ่มจาก
  
    ```bash
    hciconfig
    hciconfig <bluetooth device> up
    hcitool scan
    hcitool inq
    ```
    - Service Discovery Protocol (SDP) เป็น Protocol ของ Bluetooth ที่ใช้ในการค้นหาเครื่องต่างๆ โดยหากใช้ชุดเครื่องมือที่ชื่อว่า BlueZ จะใช้เครื่องมือที่ชื่อว่า sdptool ในการค้นหา device
    ```bash
    sdptool
    sdptool browse <MACADDRESS BLUETOOTH DEVICE>
    
    # ping to device 
    L2ping <MACADDRESS BLUETOOTH DEVICE>
    ```
    - Bluejacking เป็นเครื่องมือที่ใช้สำหรับผู้โจมตีส่งข้อมูลไปยังเครื่อง bluetooth เป้าหมายโดยไม่ทำการ pairing แต่อย่างใด หรือจ้บคู่หรือทำการเชื่อมต่อ (pairing) โดยที่อีกฝ่ายไม่ทราบถึง ซึ่งการทำ bluejacking ทำให้ผู้โจมตีสามารถส่ง message โดยไม่ถามฝั่งเครื่องเป้าหมาย รวมถึงอาจจะใช้เพื่อการทำ spoofing attack ก็เป็นได้เช่นกัน โดยการโจมตีในลักษณะ Bluejacking นั้น ใช้ Object Exchange (OBEX) protocol สำหรับการส่งภาพหรือข้อความจากเครื่องหนึ่งไปยังอีกเครื่องหนึ่ง ทั้งนี้เมื่อเป็นในลักษณะการสื่อสารแบบทางเดียว ทำให้ดูเหมือนไม่เป็นพิษภัยอะไร
    - Bluesnarfing จะคล้ายๆกับ Bluejacking แต่อันตรายกว่า โดย Bluejacking จะเป็นการโจมตีที่ส่งข้อมูลไปยังเครื่องเป้าหมาย แต่ Bluesnarfing จะเป็นการรับข้อมูลจากเครื่องเป้าหมายแทน ซึ่งอันเกิดมาจากช่วงที่ทำการ pairing จะมีช่วงที่ Bluetooth ต้องเปิดให้คนอื่นส่งข้อมูลมาเชื่อมต่อ ทำให้อุปกรณ์อื่นๆมีช่วงเวลาเล็กน้อยที่จะเข้าถึงเครื่อง Bluetooth เป้าหมายโดยไม่จำเป็นต้องทำตาม pairing process หากผู้โจมตีสามารถเข้าถึง (pairing success) ได้เรียบร้อยก็มีความเป็นไปได้ที่จะสามารถได้รับข้อมูลที่สำคัญจากเครื่องเป้าหมายได้
    - Bluebugging ปัจจุบันมีอุปกรณ์ที่คอยฟังตลอดเวลาเพื่อรับคำสั่งอย่าง Amazon’s Echo, Google’s Home, รวมถึงหูฟังที่คอยรับคำสั่งต่างๆ เพื่อไปสั่งงานมือถือของเราต่อ และอุปกรณ์อื่นๆที่คอยรับคำสั่งเหล่านั้น การโจมตี bluebugging attack จะอาศัยคอยรอการสั่งให้โทรออกจากอุปกรณ์ bluetooth ที่เชื่อมต่อกับ smartphone ของเป้าหมาย แล้วปลอมเป็นอุปกรณ์ headset เหล่านั้น แล้วหลอกให้ smart phone โทรไปตามที่กำหนด นอกเหนือจากการโทรแล้ว Bluetooth เหล่านั้นอาจจะหลอกในการสั่งงานอื่นๆแทนไม่ว่าจะเป็น ส่ง  messages, อ่าน phonebooks, และเข้าถึง calendars ก็เป็นไปได้เช่นเดียวกัน

## กฎหมายที่เกี่ยวข้อง
  
  
กฎหมายด้านไซเบอร์ในประเทศไทยมีหลายกฎหมายที่สำคัญ ยกตัวอย่างเช่น 
1. [พระราชบัญญัติว่าด้วยการกระทำความผิดเกี่ยวกับคอมพิวเตอร์ พ.ศ. ๒๕๕๐](http://sql.ldd.go.th/intraaccount/Information_Law/File/Law-10.pdf) : ซึ่งปัจุบันมีการปรับปรุงเป็น พระราชบัญญัติว่าด้วยการกระทําความผิดเกี่ยวกับคอมพิวเตอร์ (ฉบับที่ ๒) พ.ศ. ๒๕๖๐ เป็นกฎหมายที่กำหนดโทษและปรับอาญาสำหรับการกระทำความผิดทางด้านคอมพิวเตอร์ โดยเน้นไปที่การกระทำที่เกี่ยวกับความเสียหายต่อคอมพิวเตอร์และระบบคอมพิวเตอร์ รวมถึงการเข้าถึงข้อมูลหรือระบบคอมพิวเตอร์โดยไม่มีสิทธิ์ หรือทำลายข้อมูล การกระทำความผิดตามพระราชบัญญัติว่าด้วยการกระทำความผิดเกี่ยวกับคอมพิวเตอร์ (ฉบับที่ ๒) จะมีโทษอาญา และ/หรือปรับอย่างละ 2 แสนบาท สูงสุดไม่เกิน 5 ล้านบาท ขึ้นอยู่กับความรุนแรงของการกระทำความผิด 
   - Cyber Crime มีผลกระทบต่อประชาชน มีบทลงโทษที่ชัดเจน ตร. เป็นผู้ดูแล เช่น แอพดูดเงิน, การหลอกลวงด้วยวิธี Social Engineering เป็นต้น
2. [พระราชบัญญัติการรักษาความมั่นคงปลอดภัยไซเบอร์ พ.ศ. ๒๕๖๒](https://www-test.ncsa.or.th/ncsa2/wp-content/uploads/2022/12/%E0%B8%9E.%E0%B8%A3.%E0%B8%9A.-%E0%B9%84%E0%B8%8B%E0%B9%80%E0%B8%9A%E0%B8%AD%E0%B8%A3%E0%B9%8C%E0%B8%AF.pdf) - เป็นกฎหมายที่ได้รับการกำหนดขึ้นโดยรัฐบาลไทยเพื่อเพิ่มความมั่นคงปลอดภัยทางไซเบอร์ในประเทศไทย โดยมีวัตถุประสงค์เพื่อป้องกันการเข้าถึงข้อมูลส่วนตัวโดยไม่ได้รับอนุญาต การแพร่กระจายข้อมูลที่ไม่เหมาะสม การละเมิดลิขสิทธิ์  การกระทำที่เป็นอันตรายต่อความมั่นคงปลอดภัยของระบบคอมพิวเตอร์และข้อมูลในเครือข่ายอินเทอร์เน็ต
   - Cyber Security Incident กระทบต่อโครงสร้างขนาดใหญ่ 
3. [พระราชบัญญัติคุ้มครองข้อมูลส่วนบุคคล พ.ศ. ๒๕๖๒](https://ictlawcenter.etda.or.th/files/law/file/106/2487f8ffe9cf9eaf9b4decbc002eba21.PDF) - เป็นกฎหมายที่กำหนดขึ้นโดยรัฐบาลไทยเพื่อคุ้มครองความเป็นส่วนตัวและข้อมูลส่วนบุคคลของบุคคลในประเทศไทย โดยมีวัตถุประสงค์เพื่อให้บุคคลได้รับความคุ้มครองและความเป็นส่วนตัวของข้อมูลส่วนบุคคลของตนเอง การคุ้มครองข้อมูลส่วนบุคคลจะช่วยลดความเสี่ยงที่ข้อมูลส่วนบุคคลจะถูกนำไปใช้ในทางที่ไม่เหมาะสม หรือถูกนำไปใช้โดยไม่ได้รับอนุญาต

การแบ่งขอบเขตของกฎหมาย 
- Cyber Crime - พรบ.คอมฯ
- Cyber Security Incident - พรบ.ไซเบอร์ฯ
- Cyber Warfare - กฎหมายเกี่ยวกับความมั่นคง
